...
public class KafkaConsumerService:BackgroundService
{
        ...
        BotService botService)
    {
        ...
        string adminNotificationsTopic = configuration["Kafka:Topics:AdminNotifications"] ?? "notifications.register";
        string systemNotificationsTopic = configuration["Kafka:Topics:SystemNotifications"] ?? "system-notifications";

        _topics = [adminNotificationsTopic, systemNotificationsTopic];
            ...
            try
            {
                ...
                if (consumeResult != null)
                {
                    if (consumeResult.Message?.Value != null)
                    {
                        ...
                        {
                            await ProcessNotificationAsync(consumeResult.Message.Value);
                        }
   ...
   private async Task ProcessNotificationAsync(string messageJson)
    {
        try
        {
            var userRegisterMessage = JsonSerializer.Deserialize<UserRegisterMessage>(messageJson);
            ...
            _logger.LogInformation("Processing user register event {UserId}", 
                userRegisterMessage.UserId);

            await _botService.OnUserRegisterEvent(userRegisterMessage);
            _logger.LogInformation("Successfully processed user register event {UserId}", userRegisterMessage.UserId);
        }
        catch (Exception ex)
        {
